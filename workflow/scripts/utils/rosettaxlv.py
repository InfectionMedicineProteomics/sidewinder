...
def rosetta_eu_dist(pose, a, b):
    dist = sqrt(pow((pose.residue(b).atom("CB").xyz().x - pose.residue(a).atom("CB").xyz().x), 2)
                + pow((pose.residue(b).atom("CB").xyz().y -
                      pose.residue(a).atom("CB").xyz().y), 2)
...
def rosettaxlv(pose, top_XL_file, cut_off):
    ...
    for num_xl, xl in enumerate(top_XL):
        ...
        print(multiple_occ_list1)
        multiple_occ_list2 = [y for y in range(
            len(sequence)) if sequence.find(peptide2, y) == y]
        print(multiple_occ_list2)

	## finding minimum distance if multiple occurrence happened
        seq_pos_p1_k = 0
        seq_pos_p2_k = 0
        tmp_dist = 10000.0
        ...
        if ((peptide1 in sequence) and (peptide2 in sequence)):
            ...
            print(
                "The XL is not found on the protein sequence. Check each peptide to be valid!\n")
        ...
        if eulidean_dist <= cut_off:
            ...
            normal_dist_score += (10 *
                                  (1/(exp(pow((eulidean_dist-17.5), 2)/30))))
